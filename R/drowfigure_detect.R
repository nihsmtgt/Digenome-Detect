...
data=mutate(data, TFdetect=if_else(detect>th, T, F))
...
#CLscore boxplot
A=ggbase(filter(data, PAMmatch==T & distance>0 & data=="RNP(+)"))+
  geom_boxplot(aes(factor(distance), detect, fill=PAMmatch), outlier.size = 0.25)+
  guides(fill="none")+
  xlab("Distance")+
  ylab("CL-score")+
  lims(x=factor(c(1:6)), y=c(0,300))

B=ggbase(filter(data, PAMmatch==F & distance>0 & data=="RNP(+)"))+
  geom_boxplot(aes(factor(distance), detect, fill=PAMmatch), outlier.size = 0.25)+
  guides(fill="none")+
  xlab("Distance")+
  ylab("CL-score")+
...
D
ggsave(paste0(output, "_threshold.svg"), width = 9, height = 9/1.618, units = "cm")
...
r=group_by(filter(data,detect>th & data=="RNP(+)"), PAMmatch) |> 
  count(distance)  |> 
...
r2=group_by(filter(data,detect>th & data=="RNP(+)"), PAMmatch_2) |> 
  count(distance_2) |> 
  replace_na(list("FALSE"=0, "TRUE"=0))
